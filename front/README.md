...
- 选择 `mbddf.di_read` 后，首页按后端 descriptor 显示 16 路 DUT DI 刺激开关、物理 High/Low、`di_state[0]` 回读匹配状态和只读 `di_state[1]` 诊断位图；快速操作会合并并串行发送带 revision 的请求，失败时回滚到后端权威状态，且可一键恢复安全态。
