...
XDMA event 就绪后，Transport 会读取并消费 4 字节事件计数；关闭 event fd 本身不会
清除驱动中的 pending 计数。COM 接收若先遇到上一次运行遗留的事件而 D0 尚未完成，
会在调用方给定的总超时内继续等待当前帧，因此连续启动两次回环不会把旧事件误判为
本次完成。每个 `_events_N` 必须保持单消费者，不得同时运行两个占用同一 COM event
节点的进程。
