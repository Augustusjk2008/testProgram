...
二进制。工程没有 C++ host/native 构建路径；`test_pyqt` 的 Windows Python 测试是明确
例外，见[测试快速说明](../tests/README.md#pc-串口工具测试)。
...
由独立 PC/夹具在被测 COM 原样回发 114 字节。旧 PyQt 只通过 COM3 发产品协议，不能作为
外部 ECHO 回送端或验收证据。
