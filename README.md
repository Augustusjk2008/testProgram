...
当前版本提供 MB_DDF_v2 的十一项配置驱动测试：`SYSTEM_STATUS`、`ELEC_HEALTH_STATUS`、`BUS_LOOP_TEST`、`BUS_ECHO_TEST`、`MEMPERF_TEST`、`SPI_FLASH_TEST`、`DH_PULSE_CONFIG`、带 STOP 清理的 `TIMER_JITTER`、`DI_READ`、`IMU_STREAM` 和 `HELM_STREAM`。BUS 只测试 COM1/COM2/COM4：LOOP 使用 DUT 内部回环，ECHO 由 PC 周期调度并通过配置中的三个独立 Qt 串口与 DUT 完成 114 字节往返；COM3 控制口、网口和 SPI Flash 均不进入 BUS 链路。一致性结果只投影紧凑摘要，避免 114 个字节字段进入实时曲线。一次性 runner、分步 TUI、Qt Widgets GUI、本机 WebSocket 后端和浏览器遥测控制台共享同一个应用控制器；当前宿主开发与验证以 Web 链路为主基线。BIZ 支持单次、PC 主动周期和设备主动持续回告三种通用语义。舵机连续实测由 DUT 以 1 ms 周期生成指令，经 DDS 与用户独立启停的 `MB_DDF_v2_HelmControl` 交互；它与 `HELM_BOARD_TEST` 并列存在且没有生命周期、互斥或忙状态绑定。板端当前通讯基线是串口，PC 端仍保留 UDP 控制资源，用于本机模拟和后续网口扩展。BUS ECHO、IMU、DDS 舵机和其他新增硬件路径尚无真实板端验收证据。
