...
| 层 | 负责 | 不负责 |
| --- | --- | --- |
| UI / 应用 | 配置目录与持久化文档、算法参数 Schema 投影和本次覆盖合并、组合生命周期、DTO/动作/快照/样本适配、有序关闭；按算法/参数条件组合板级夹具；后处理的身份、封存、后台编排、取消与小型结果投影 | 自行定义产品参数语义或范围、产品协议解释、直接打开 DUT 或测试设备、绕过 BIZ 执行判定、在浏览器从实时缓存重算权威性能 |
