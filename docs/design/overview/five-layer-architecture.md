...
控制资源按 HAL 部署配置中的 `control.resourceId` 和资源 `providerId` 选择串口或 UDP；Qt 标准接口不经过 Vendor Adapter。非控制资源由 `AdapterRouter` 按设备 `adapterId` 路由：Mock 设备使用 `MockAdapter`，配置 `providerId = vendor.cabi` 或 `libraryPath` 的设备使用实际调用 ABI v1 的 `CAbiAdapter`。Vendor C ABI 初始化只接收对应 Adapter 的驱动级配置；`HalService::openDevice()` 再为目标设备从资源映射构造 `schema=hwtest.adapter-device-open`、`version=1` 的单设备投影，并以 C ABI 的 `openOptionsJson` 传入。投影包含逻辑/物理身份、通道、任务档案和该设备的 safe state。后端在首次打开相应设备时才初始化，最后一个会话释放后关闭并卸载。`ni.daqmx` 是 `src/adapters/ni_daqmx/` 的 PXI-6259 原生 ABI v1 DLL 和默认关闭的 `HWTEST_ENABLE_NI_DAQMX` 可选构建；部署的 `libraryPath` 必须指向该 DLL，真实设备身份/路径仍须另行配置。HAL 动态 fixture 回归锁定了 driver-only 初始化与版本化 open projection 的交接；NI Fake 进一步覆盖 Adapter 软件路径。纯协议 golden 测试仍可注入 `SystemStatusSimulator`。
...
- `[当前实现]` 通用 C ABI、DI 刺激、多 Adapter、可选任务 ABI 和安全态的自动化证据使用 Mock、`IHalDevice` 测试替身或动态 Fake Adapter DLL；`NiDaqmxAdapterFakeTest` 还以同一 PXI-6259 Adapter 源码链接 Fake `NIDAQmx` API。两类 Fake 均不证明真实 NI SDK、实际 PXI-6259、接线、物理输出或物理安全态。
...
- `front/` 是独立的 React 19、TypeScript、Vite、Tailwind CSS 和 uPlot 工程，不进入宿主 CMake，也不包含后端、数据库或 DUT I/O。DI 协议类型、WebSocket transport、SessionProvider、纯命令队列和 `DigitalStimulusPanel` 已接入总览页：面板做 32 ms 同开关合并、单飞行串行、revision/回滚、active-low、`di_state[0]`/`di_state[1]` 回读和 settling 提示。它是浏览器控制面实现，不是 PXI-6259 真机验收。
...
- `[当前实现]` PXI-6259 NI-DAQmx 软件路径已实现但尚无真机验收：源码、默认关闭的可选 CMake 目标和 Fake SDK CTest 已存在；Router 已按驱动级 settings 初始化 Vendor Adapter，设备身份、资源和 safe state 在 open projection 中传递。模板将 MAX 名称置于设备 `properties.vendor.ni.deviceName`，并以 `CONFIGURE_ME` 序列号在 open projection 解析时阻止误开。`[目标契约-未实现]` 真实 PXI-6259 的接线、物理输出/回读、长时、拔插和物理安全态验收仍待独立证据。
