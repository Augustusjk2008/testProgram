...
| tests/algorithm/ | hwtest_algorithm_tests | 16 | 133 | MB_DDF CSV、1..255 payload、F32 来源类型、流式控制传输、统一串口回环/多轮回显路由与有界诊断、运行参数 Schema/归一化、任务范围控制连接与同 worker 停止收尾、固定命令、DO_WRITE/HELM_BOARD_TEST 板级流程、DH 点火有限流、惯测/舵机设备流、IMU 宿主时间戳间隔、DI stimulus；以及后处理公共端口、版本化捕获、五种舵机波形、扫频局部同步估计、状态归约、取消、有限 JSON 和可追溯 fixture |
| tests/app/ | hwtest_app_tests / hwtest_gui_tests / hwtest_web_tests | 16 | 185 | 共享启动/控制器、配置目录十三项发现、串口辅助端口白名单、DI/DO prepare fixture 生命周期、板级 fixture 按模式绑定、设备流本地时间配置、TUI/GUI/WebSocket、DI 与运行能力、零间隔参数、连续 TXT 保存、DH 不可停止有限流与重连、惯测/舵机 Qt UDP；以及分析资源配置、STOP 双栅栏和尾样本、完整舵机捕获—分析—查询链、Web v1 capability/批量遥测协商、板级 `rawData` 投影、全局安全序号、FIFO/背压和出站边界 |
...
| 合计 | 7 个 GoogleTest + 1 个 Fake CTest | 52 | 425 | 51 个含 GoogleTest 定义的源文件及一个非 GoogleTest 测试源 |

425 是当前测试源码中的 GoogleTest 定义数。按当前 CMake 的七个动态发现目标、10 条应用入口/脚本进程测试和 1 条 `NiDaqmxAdapterFakeTest` 计算，完整构建并完成测试发现后应有 436 条 CTest；该静态清单不是本次执行结果。清单数量本身不表示通过，执行证据见下述按时间记录。
...
2026-07-30 新增独立 `DH_IGNITE_STREAM` 有限设备流并完成审阅修复后，宿主使用仓库内 37 份协议快照完成 Debug 与 Release 全量构建/CTest；两个配置各有 393 条注册项，均为 392 项执行通过、1 项条件跳过、0 失败。跳过项仍是缺少外部导入附件的 `TestConfigManagerTest.ImportedColleagueSampleLoadsAndBuildsThroughMigrationBoundary`，不计为通过。新增自动化覆盖 `delay_frames` 替换旧时间延迟、PC 仅编码级校验、DUT 写前业务校验、动态基线、一次点火、最小采样间隔、基线失败不点火、点火后采集错误继续、发送失败终止、U16 响应序号回绕、首帧业务拒绝分类、理想时间戳、显式保存、不可停止 capability、Web 断开立即 detach/重连及自然完成。浏览器前端为 18 个测试文件、88/88 Vitest 通过并完成单文件生产构建；旧 Windows PyQt 为 228/228，DUT 工具为 20/20，37 份 CSV 严格校验通过，AArch64 Debug `hw_tests` 画像交叉构建 `MB_DDF_HW_Tests` 成功。Release 仅出现既有 Qt 5/MSVC `C4996/STL4043` 弃用告警，PyQt 仅出现既有依赖弃用告警。本轮未部署或运行 AArch64 测试，也未连接真实 DH 台架，因此不构成点火输出、硬件门控、2500 us 实时时序或物理安全验收。

2026-07-30 完成 `DO_WRITE` 与 `HELM_BOARD_TEST` 根宿主板级测试后，使用仓库内 37 份协议快照顺序执行 Debug 与 Release 全量 CTest；两个配置各有 415 条注册项，均为 414 项执行通过、1 项条件跳过、0 失败。跳过项仍是缺少外部导入附件的 `TestConfigManagerTest.ImportedColleagueSampleLoadsAndBuildsThroughMigrationBoundary`，不计为通过。浏览器前端 22 个测试文件的 99/99 Vitest 通过，TypeScript/Vite 单文件生产构建成功。新增证据覆盖五步 DO 闭环、手动不开 PXI、自动方向/PWM/反馈扫点、精确采样块与非有限数拒绝、四路 AO 尽力清零、版本化 `rawData.boardTest` 与三路实际 DI fixture 界面。全部硬件路径仍使用 Mock/Fake NIDAQmx，未配置 `CONFIGURE_ME`、未打开真实 PXI-6259/PXI-6733 或 DUT，因此不构成接线、电平、采样精度、AO 输出或物理安全收尾验收。

2026-07-30 完成本轮测试项与页面可用性收敛后，使用仓库内 37 份协议快照分别完成 Debug、Release 全量构建与 CTest；两个配置各有 436 条注册项，均为 435 项执行通过、1 项条件跳过、0 失败。跳过项仍是缺少外部导入附件的 `TestConfigManagerTest.ImportedColleagueSampleLoadsAndBuildsThroughMigrationBoundary`，不计为通过。浏览器前端为 25 个测试文件、116/116 Vitest 通过，TypeScript/Vite 单文件生产构建成功。新增证据覆盖统一串口测试的回环/回显路由、COM1/2/4 与统一次数边界、回显辅助串口枚举白名单/主辅冲突、每轮 5 秒 deadline、回环 DUT 完成数归约和最多 16 条终态诊断；DI/DO 在 prepare 阶段 fail-fast；DO 用户 16 位配置、DO5/DO6 禁用、DUT 完整回读、PXI-6259 仅验证 DO2/DO1且不自动复位；DH 使能复选框、23 路全选/全不选与“等待帧数”；定时器精确八桶分布、遥测大字诊断移除及 PC 周期间隔控件收敛。Release 仅出现既有 Qt 5/MSVC `C4996/STL4043` 弃用警告。未连接真实 COM1/COM2/COM4、PXI-6259、DH 台架或目标板，以上 Fake/脚本化/浏览器自动化不构成真机验收。

51 个含 GoogleTest 定义的源文件使用 `*_test.cpp` 命名，共 425 个 GoogleTest 定义；另有 1 个 NI Fake 自定义 main 测试源。四个 HAL DLL fixture、Fake NIDAQmx 库/头、GUI/Web 自定义 GoogleTest 入口、应用测试支持库和测试 helper 不包含 GoogleTest 定义，不计入 425。

浏览器前端当前有 25 个 `*.test.ts`/`*.test.tsx` 文件、116 条 Vitest。除既有协议、参数、遥测缓冲/图组、DI 和性能结果行为外，新增覆盖旧 `sample`/新 `sampleBatch` 兼容、全局序号重复/倒序/缺口诊断、50,000 点淘汰统计、像素级 min/max 抽样、连接 epoch 隔离、零间隔参数、descriptor `stoppable` 兼容/类型边界、不可停止运行态控件、`persistValues=false` 的 localStorage 边界、统一串口辅助端口选择、DH/DO 专用参数编辑、定时器八桶终态恢复，以及 `rawData.boardTest` 防御解析、两项算法专用导航、manual/automatic/用户配置结果呈现和三路实际 DI fixture 的动态面板。它们由 `npm test` 独立运行，不计入上述 436 条 CTest。

## 2. 当前覆盖与条件资产

| 目标 | 当前已覆盖的行为 | 证据边界 |
| --- | --- | --- |
| HAL | 错误映射、严格多设备资源映射、会话、Mock AD/DA/DI/DO、宿主串口枚举、串口 echo、CANFD loopback、控制资源路由、Qt UDP 回环和 timeout；动态 Fake ABI v1 的实际装载/数字批写/状态映射/缺符号、AdapterRouter 惰性多后端路由、逻辑 alias 映射、driver-only 初始化、版本化单设备 open projection、`HalService -> AdapterRouter -> CAbiAdapter -> NI parser/Fake DAQmx` 组合链、核心 ABI v1 的可选 task API 兼容、C ABI I/O/close 串行化、短读任务块、任务先 stop/close 再 safe state，以及 close 错误消费 handle | 自动化测试中的串口枚举不打开设备；动态 DLL 是 `tests/hal/fixtures` 的 Fake，Qt UDP 仅是本机 Provider 证据；COM3 实机结果属于下述独立手工 smoke，不是默认 CTest、NI SDK 或 PXI-6259/PXI-6733 证据 |
| 原生 NI-DAQmx Adapter | 同一生产源码和独立 `ni_daqmx_config.*` 的 Fake 构建；PXI-6259 identity/open projection/占位 serial/topology/速率边界，按需 AI/AO/DI/DO，有限/连续任务路径，内部/外部时钟、start/reference/pause 触发、边沿计数/频率脉冲、短读、overflow/underflow、错误注入、任务状态、安全态/stop/clear 顺序；PXI-6733 AO-only `ao0..ao7` profile 和零 AO safe state；Vendor C ABI driver-only 初始化与单设备 open projection 交接；默认关闭的真实 SDK 构建路径 | `NiDaqmxAdapterFakeTest` 链接仓库 Fake `NIDAQmx`；独立可选 DLL 构建也使用 Fake 头/导入库。两者均不代表已安装 NI SDK、实际 PXI-6259/PXI-6733、MAX 身份、接线、电平兼容、物理输出、触发时序或安全态验收 |
...
| 算法 | 帧编解码、CSV 无效输入、控制传输、运行参数 Schema、固定命令与配置驱动交换、统一串口回环/多轮回显路由、DO_WRITE 用户单掩码与两路 PXI 物理验证、HELM_BOARD_TEST manual 单次交换与 automatic 5/36/44 点、判据失败继续扫描及 AO 清零失败 Error、DH 点火有限流、惯测/舵机设备流和 DI stimulus；后处理公共 API 不泄露产品字段，捕获按小端版本化格式保存 DDS 时间、数值与诊断，封存后不可变；独立 fixture/manifest 覆盖恒值、正弦、方波、三角波、一阶/二阶/纯时延扫频、正反向与提前停止，另覆盖激励阈值、相位符号、尾段、状态归约、取消和有限 JSON | fixture 由不调用 C++ 分析器的 Python 标准库脚本生成并记录 SHA-256；合成曲线只能证明数值和边界实现。DH/IMU/舵机设备流仍以脚本化传输为主，串口与板级自动化使用 Fake/Mock；本机模拟不能证明 COM1/COM2/COM4、DH 真实点火、DDS、真实舵机、921600、400 Hz/1 kHz、NI/PXI-6259/PXI-6733 或目标板行为 |
| 应用/TUI/GUI/WebSocket | 既有共享启动、控制资源、DI、三种运行模式、零间隔参数、TXT 保存及 GUI/Web 非阻塞收尾；当前十三项配置发现、串口辅助端口白名单选择、DI/DO 在 prepare 打开 PXI、HELM manual 不开 PXI、automatic 缺 PXI-6259/PXI-6733 fail-closed、6259 数字/采样与 6733 批量 AO fixture 绑定、结果快照投影；DH 增加显式不可停止 capability、动作拒绝、断开 detach、重连和自然完成；后处理增加 start 前存储预检、TXT→捕获→公开样本顺序、STOP 封存/硬件收尾双栅栏、queued 写门禁、工作线程进度/取消/join、资源/结果/摘要限制和当前身份只读查询；Web v1 覆盖 capability、按连接协商 `single`/`batch-v1`、批量时间/条数/字节门限、全局安全序号、任务切换与关键事件前冲刷、统一 FIFO、软背压和硬上限失败 | 完整控制器用例加载真实 DH/舵机配置和协议 CSV，经本机 Qt UDP 验证 DH 有限流生命周期，并产生 HELM 样本完成捕获—分析—文件—查询，证明宿主软件组合链；串口、板级 fixture、Web 自动化和本机客户端仍不构成真实串口回显、DH 写入、PXI-6259/PXI-6733、AO 物理清零或 1 kHz/2 kHz、60 秒真实浏览器压力证据。默认自动化不能外推到真实网口、真实 DH/舵机、其他 DUT 或长期稳定性 |
| 浏览器前端 | 类型化协议、参数表单、`stoppable` 兼容与不可停止控件、`persistValues=false` localStorage 边界、旧 `sample`/新 `sampleBatch`、按连接 epoch 丢弃迟到事件、全局序号完整性诊断、50,000 点有界 store、累计接收/淘汰统计、像素级 min/max 查询、遥测 store 与低频 Context 隔离、约 10 Hz UI 提交，以及既有图组、DI、性能结果、`rawData.boardTest` 防御解析、两项算法专用导航和 manual/automatic 板级结果呈现；TypeScript/Vite 单文件生产构建 | 性能页只消费后端 `snapshot.analysis`/`analysisResult`，板级页只消费当前 `rawData.boardTest`；Vitest 和构建不证明真实 WebSocket、DH、DDS/舵机/PXI-6259/PXI-6733、视觉效果、长期浏览器稳定性或真机性能，1 kHz/2 kHz 压力和 Chrome Performance 仍需用户授权启动服务后执行 |
...
| 4. Vendor Adapter | 厂家 Adapter DLL/SDK 经 C ABI 接入 | 通用 C ABI 与可选原生 NI-DAQmx Adapter 已实现，自动化仅有 Fake | `CAbiAdapter` 已实际装载/调用核心 ABI v1，并有惰性多 Adapter、数字批写和可选 task ABI 兼容回归；`hwtest_adapter_ni_daqmx` 具有默认关闭、待具备 SDK 后才可配置的构建路径，以及覆盖 PXI-6259/PXI-6733 软件任务路径的 Fake NIDAQmx CTest。Fake 不能证明已安装 SDK 或真实厂家设备；没有 `hardware` 真机标签 |
| 5. 真实硬件 | 隔离台架上的真实目标和测试设备 | 部分手工证据 | 已有上述授权 COM3 SYSTEM_STATUS/ELEC_HEALTH_STATUS 历史及 QThread 迁移后复测；NI-DAQmx 路径已实现但没有 DI/NI/PXI-6259/PXI-6733 真机、宿主 CTest hardware target、长期稳定性、异常收尾或完整物理安全验收 |
...
- 修改 Qt Provider、通用 C ABI、原生 Vendor Adapter 或真实硬件路径时，必须新增相应级别的隔离测试；当前 Qt UDP 有本机隔离测试，通用 C ABI 已有动态 Fake DLL、多 Adapter 懒加载、状态映射、数字批写和可选 task ABI 回归，PXI-6259/PXI-6733 NI-DAQmx 已有可选 SDK 构建路径及 `NiDaqmxAdapterFakeTest`，BIZ 已有 Qt dispatcher/计时器注册回归，Qt 串口已有带历史告警的早期 smoke、无响应无告警诊断和迁移后成功无告警的手工复测；CTest `hardware` 标签、自动化异常路径和全面真实硬件验收仍未实现。
...
- 修改 DO_WRITE/HELM_BOARD_TEST、板级 fixture、`rawData.boardTest` 或 `persistValues` 时，必须覆盖 DO 16 位参数与 DO5/DO6 禁止、DO3/DO4 安全默认、单掩码完整 DUT 回读、只对 bit2/bit1 的 6259 外部验证、未接外部回读的 UI 标识、DO 完成/停止/断开零复位写入，以及 HELM manual/automatic 模式边界、automatic 的 5/36/44 点与失败继续扫描、AO 清零失败、manual 零夹具访问、夹具缺失 fail-closed、v1 防御解析/专用导航和 `persistValues=false` 不读写 localStorage。上述回归只能作为 Fake/Mock/前端证据；真实 PXI-6259/PXI-6733 验收还须满足 HAL 契约的隔离台架前置条件。
...
当前 `TestApplicationController` 的十三份现行配置使用十三个算法 ID（`mbddf.system_status`、`mbddf.elec_health_status`、`mbddf.serial_test`、`mbddf.memperf`、`mbddf.spi_flash`、`mbddf.dh_pulse_config`、`mbddf.dh_ignite_stream`、`mbddf.timer_jitter`、`mbddf.di_read`、`mbddf.do_write`、`mbddf.helm_board_test`、`mbddf.imu_stream`、`mbddf.helm_stream`），注册表另保留 `mbddf.bus_loop`/`mbddf.bus_echo` 旧 ID，并要求每份配置恰好一个启用步骤。串口、SPI Flash、DH 脉宽、DO_WRITE 与 HELM_BOARD_TEST 只允许 `single`，系统状态、电气健康、内存、定时器与 DI 按配置允许 `single`/`pc_periodic`，DH 点火、惯测与舵机连续实测只允许 `device_stream`；HELM_BOARD_TEST 的 `automatic`/`manual` 是运行参数。DH 点火还必须覆盖 PC 编码范围与 DUT 业务范围分离、动态 `delay_frames`、零硬件写入拒绝、不可停止 capability、断开 detach/重连和自然完成；不得把 Fake、Qt UDP、主机侧或交叉构建结果记为真实点火验收。串口自动化必须覆盖 0/1/3 白名单、COM3 拒绝、统一次数边界、内部 `loopback=true`、外部 114 字节多轮短读/回写/双向一致性、单轮 5 秒超时、轮间停止、本地端口白名单及主辅同口拒绝；不得把 Fake/主机侧结果记为 COM1/2/4 真机验收。DO_WRITE/HELM_BOARD_TEST 自动化必须覆盖其单点或模式边界、错误分类与 AO 收尾，但不得把 Fake/Mock/PXI 模板结果记为板级真机验收。以下规则是新增测试项目的合入门禁，不得据此宣称任意产品协议都已实现：
...
上述配置会在常规目标之外生成 `hwtest_adapter_ni_daqmx`；真实 PXI-6259/PXI-6733 的部署、MAX 身份、接线、测试和安全验收必须在授权的隔离台架另行记录，不能由该命令或 Fake CTest 得出。
