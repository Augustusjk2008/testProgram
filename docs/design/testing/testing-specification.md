...
- 合计为 55 个测试源文件：54 个含 GoogleTest 定义的源文件和 1 个 NI Fake 自定义 main；GoogleTest 定义数为 471。
- 在一个已完成发现的构建树中，当前应有 482 条 CTest：471 条动态 GoogleTest、10 条应用进程测试和 1 条 NI Fake CTest。`ctest -N` 只核对该清单，不证明通过。
- 浏览器前端当前有 35 个 `*.test.ts`/`*.test.tsx` 文件和 150 条 Vitest；它们不计入 482 条 CTest。
