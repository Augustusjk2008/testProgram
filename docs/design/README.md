...
| 应用 | `src/app/` | `hwtest_app_core` 统一组合生命周期；`hwtest_pc_runner`、`hwtest_tui`、`hwtest_gui` 与回环 `hwtest_web` 是独立入口；`front/` 尚无浏览器实现 |
| 测试 | `tests/hal/`、`tests/log/`、`tests/biz/`、`tests/algorithm/`、`tests/app/` | 七个 GoogleTest 目标，经 CTest 注册 |

`[当前实现]` 仓库已有行式 TUI、Qt Widgets GUI 和 WebSocket 后端，但 `front/` 没有浏览器页面，仍不能宣称完整 Web UI 已实现。TCP Provider、真实厂家链和真实硬件验收也未实现。Qt SerialPort/Network 已用于控制通道，UDP 已有本机模拟目标闭环，真实串口尚未联调。测试目标、源码清单和统计口径统一见 `testing/testing-specification.md`。
