...
> **历史归档**：这是 2026-07-28 收敛前的总览全文，其中包含已迁移到契约、测试规范和用户指南的重复内容。现行总览见 `../overview/five-layer-architecture.md`。
...
- 配置、日志、报告和连续样本文件 I/O 不属于上述生产硬件/通讯 I/O 规则。连续样本 recorder 位于应用组合层，消费应用 DTO；它不反向进入 BIZ、算法或 HAL，也不改变产品判定。
