...
本文定义 `hwtest_web` 与未来浏览器前端之间的版本 1 JSON 协议。`hwtest_web` 是与 TUI、Qt Widgets GUI 并列的独立应用进程；它只把 WebSocket 消息适配为 `TestApplicationController` 动作和快照，不解释产品协议，也不直接访问 BIZ、算法、HAL 或 DUT 通讯对象。

`[当前实现]` 服务器使用 Qt WebSockets，仅监听 IPv4 回环地址 `127.0.0.1`，默认端口为 `8765`，唯一资源路径为 `/ws`。它不提供 HTTP、静态文件、TLS、数据库、登录或远程访问。仓库根目录的 `front/` 只是未来浏览器前端的预留目录；本文不表示浏览器前端已经实现。
...
- 每个有效请求恰好产生一个相同 `id` 的 reply。快照是独立异步事件，可以出现在请求与 reply 之间。
...
无参数动作不得从 `params` 读取行为配置。`load` 尤其不得读取 `testConfigPath`、`halConfigPath` 或其他客户端路径字段。
...
3. `snapshotChanged` 事件立即形成完整 snapshot 消息，因此它可以先于触发该变化的动作 reply 到达。
