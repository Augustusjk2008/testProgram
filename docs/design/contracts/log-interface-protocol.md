...
- 五层不直接写日志存储；当前报告只读取 `TestResult`，不依赖日志摘要或负责收集日志。
...
该词表是契约，不表示所有来源均已有生产者；当前已有行式 TUI、Qt GUI、WebSocket 后端和 `front/` 浏览器遥测控制台，但它们尚未形成独立的 UI/Web 日志生产或订阅链，真实厂家链也仍未实现。浏览器诊断页只显示客户端会话事件、快照、样本和帧投影，不构成远程日志能力。

- `[当前实现]` BIZ 为一次测试任务创建非空 `requestId`；算法通过 `IByteTransport::setRequestId()` 将其传给 HAL-backed transport，后者复制到打开、读、写和关闭的每个 `OperationOptions`。同一操作链的其他生产者也应复用该值。
