...
- 算法位于 `src/algorithm/`，目标为 `hwtest_algorithm_mbddf`，提供 MB_DDF CSV 编解码和十四项已注册算法：`mbddf.system_status`、`mbddf.elec_health_status`、`mbddf.bus_loop`、`mbddf.bus_echo`、`mbddf.memperf`、`mbddf.spi_flash`、`mbddf.dh_pulse_config`、`mbddf.dh_ignite_stream`、`mbddf.timer_jitter`、`mbddf.di_read`、`mbddf.do_write`、`mbddf.helm_board_test`、`mbddf.imu_stream`、`mbddf.helm_stream`。
- 十四份 `configs/mbddf_*.testcfg.json` 中，BUS LOOP、SPI Flash、DH 脉宽、DO_WRITE 和 HELM_BOARD_TEST 只声明 `single`，BUS ECHO 只声明 `pc_periodic`，系统状态、电气健康、内存、定时器和 DI 声明 `single` 与 `pc_periodic`，DH 点火、惯测与舵机连续实测只声明 `device_stream`。BIZ 提供 `single`、`pc_periodic` 和 `device_stream` 三种通用运行语义，配置能力以各 JSON 的 `reportFields.supportedRunModes` 为准；同一配置不得同时声明 `pc_periodic` 与 `device_stream`，字段缺失时只安全回退到 `single`，应用控制器拒绝启动未声明的模式。HELM_BOARD_TEST 的 `automatic`/`manual` 是算法运行参数，不是 BIZ run mode。
- 电气健康以设备 `status`/`err_code` 判定；SPI Flash 写入固定隔离测试区并保留写入结果；`TIMER_JITTER` 在结束阶段发送 STOP 清理。
- BUS 测试只允许 link 0/1/3，对应 COM1/COM2/COM4；COM3 是产品控制口并被拒绝，网口和 SPI Flash 不属于 BUS 测试。LOOP 由 DUT 配置内部 `loopback=true`；ECHO 每个 `pc_periodic` 周期经控制口发起一次命令，DUT 在所选串口发送固定 114 字节，PC 的独立 `qt.serial` 辅助资源原样回送并逐字节校验，整轮无响应截止时间为 5 秒。辅助串口端口名由 `configs/mbddf_pc_hal.json` 配置且不投影为可选主控制口；自动化不构成真机回显验收。
...
- `mbddf.do_write` 固定按 `0x0018 -> 0x001C -> 0x0018 -> 0x001A -> 0x0018` 执行 `DO_WRITE 04/02`；每步确认 DUT 应答后等待 100 ms，再由 PXI-6259 的 P0.3/P0.4（`DUT_TX_ENABLE_SENSE`/`DUT_ATTENUATOR_SENSE`）读回闭环。协议、DUT 或 HAL 错误为 Error；物理不一致为 Fail 但继续生成完整五步矩阵。
...
- `hwtest_web` 提供回环 WebSocket v1 服务；`front/` 提供独立的 React/Vite 遥测控制台，支持配置选择、运行控制、两种连续模式、算法运行参数编辑、完整 descriptor 测量列保存、动态测量字段和当前配置的 DI 刺激/回读（WebSocket v1 最多 16 路）。运行期覆盖只作用于本次启动，浏览器按 `configId + runParameterSchemaVersion` 保存编辑值；descriptor 参数显式 `persistValues=false` 时既不从也不向 localStorage 读写，字段缺失仍兼容为 true。保存目录只由后端配置，曲线选择不改变保存列。`mbddf.do_write` 与 `mbddf.helm_board_test` 专用页面只消费当前完整快照的版本化 `rawData.boardTest` v1，显示 DO/方向矩阵、自动 PWM/反馈的指令/实测/误差/容差和摘要或手动一次响应；它不是 TXT、跨进程持久化或真机验收。`mbddf.helm_stream` 还投影 `postRunAnalysis` capability、`snapshot.analysis` 小型摘要和只读 `analysisResult` 按通道结果；浏览器性能页只消费后端投影，不以 50,000 点实时环形缓存计算性能。
