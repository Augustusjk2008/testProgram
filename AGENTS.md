...
- `src/adapters/ni_daqmx/` 在 `HWTEST_ENABLE_NI_DAQMX=ON` 时构建支持 PXI-6259 与 PXI-6733 profile 的 NI-DAQmx Adapter。核心 ABI v1 保持稳定，可选 task ABI v1 与 `ISampleTaskIo` 提供 AI/AO/DI/DO、有限/连续采样、时钟、start/reference/pause 触发、边沿计数和频率脉冲输出；PXI-6733 当前只接受 `ao0..ao7` 模拟输出且 safe state 必须为 `0.0 V`。Fake NI-DAQmx 只覆盖软件路径。
...
- 算法位于 `src/algorithm/`，目标为 `hwtest_algorithm_mbddf`，提供 MB_DDF CSV 编解码。十三份当前配置使用 `mbddf.system_status`、`mbddf.elec_health_status`、`mbddf.serial_test`、`mbddf.memperf`、`mbddf.spi_flash`、`mbddf.dh_pulse_config`、`mbddf.dh_ignite_stream`、`mbddf.timer_jitter`、`mbddf.di_read`、`mbddf.do_write`、`mbddf.helm_board_test`、`mbddf.imu_stream` 和 `mbddf.helm_stream`；注册表额外保留 `mbddf.bus_loop`、`mbddf.bus_echo` 两个旧算法 ID，用于受控旧配置兼容，不再提供对应的当前配置入口。
- 十三份 `configs/mbddf_*.testcfg.json` 中，串口、SPI Flash、DH 脉宽、数字量输出和 HELM_BOARD_TEST 只声明 `single`，系统状态、电气健康、内存、定时器和 DI 声明 `single` 与 `pc_periodic`，DH 点火、惯测与舵机连续实测只声明 `device_stream`。BIZ 提供 `single`、`pc_periodic` 和 `device_stream` 三种通用运行语义，配置能力以各 JSON 的 `reportFields.supportedRunModes` 为准；同一配置不得同时声明 `pc_periodic` 与 `device_stream`，字段缺失时只安全回退到 `single`，应用控制器拒绝启动未声明的模式。HELM_BOARD_TEST 的 `automatic`/`manual` 是算法运行参数，不是 BIZ run mode。
- 电气健康以设备 `status`/`err_code` 判定；SPI Flash 写入固定隔离测试区并保留写入结果；`TIMER_JITTER` 在结束阶段发送 STOP 清理。
- “串口测试”只允许 link 0/1/3，对应 COM1/COM2/COM4；COM3 是产品控制口并被拒绝，网口和 SPI Flash 不属于该测试。配置只声明 `single`，运行参数以 `test_mode` 选择回环或回显，并以统一 `cycle_count=1..100000`（默认 1000）表达循环次数。回环由 DUT 配置内部 `loopback=true`，一次 `03/01` 请求携带全部次数；回显由复合算法在同一次 BIZ 运行中执行指定轮数的 `03/02`，每轮 DUT 在所选串口发送固定 114 字节，PC 通过用户选择的独立 `qt.serial` 本地串口原样回送并逐字节校验，单轮无响应截止时间为 5 秒。辅助串口不得与主控制串口相同；自动化不构成真机回显验收。
- DH 点火有限流复用 `DH_CONTROL 06/02`，但与 `mbddf.dh_pulse_config` 独立。`delay_frames=N` 表示先回告 N 帧只读基线，再在下一帧采样前点火一次；`report_count`、`interval_us` 和 `delay_frames` 由 PC 发送，PC 只校验编码范围，DUT 在硬件写入前校验业务约束。该配置的 descriptor 声明 `stoppable=false`：请求受理后不发送 STOP/ABORT、不自动复位，Web stop/pause/resume/quit 被拒绝，断开仅分离客户端，有限流继续到自然终态并允许重连。
...
- `mbddf.do_write` 由 PC 端 16 路布尔运行参数生成一次 `DO_WRITE 04/02` 掩码；DO5/DO6 固定为 0，低有效的 DO3/DO4 默认原始位为 1。完整 `applied_state` 必须等于指令值；应答后等待 100 ms，再由 PXI-6259 P0.3/P0.4（`DUT_TX_ENABLE_SENSE`/`DUT_ATTENUATOR_SENSE`）只对 bit2/bit1 执行外部物理读回，其他位明确只有 DUT 回读证据。协议、DUT 或 HAL 错误为 Error，回读不一致为 Fail。PXI-6259 与 DI 刺激设备一样在 `prepare` 阶段打开；测试完成、停止、断开或退出均不额外复位 DUT DO，产品保持最后一次用户配置状态。
- `mbddf.helm_board_test` 只支持 `single`。`manual` 只发送一次 `HELM_BOARD_TEST 07/02`，不打开或读写 PXI-6259/PXI-6733；`automatic` 独立执行方向掩码 `0/1/2/4/8`、四通道各九个 PWM 点（1/5/10/25/50/75/90/95/99 %，PXI-6259 1.25 MS/s、7500 样本、`±1 pp`）和四通道反馈点（PXI-6733 AO0..3，0..5 V、0.5 V 步进、`±0.05 V`）。自动模式从开始到完成、Fail、停止或 Error 都尽力将 AO 写回 `0 V`；物理端点、`CONFIGURE_ME` 门禁和真机前置条件以 HAL 契约为准。
...
- `hwtest_web` 提供回环 WebSocket v1 服务；`front/` 提供独立的 React/Vite 遥测控制台，支持配置选择、运行控制、两种连续模式、算法运行参数编辑、完整 descriptor 测量列保存、动态测量字段和当前配置的 DI 刺激/回读（WebSocket v1 最多 16 路）。串口回显可从系统枚举结果选择独立 PC 本地串口；DH 点火使能使用复选框且 23 路通道支持全选/全不选；定时器页面按精确半开区间显示八桶分布。运行期覆盖只作用于本次启动，浏览器按 `configId + runParameterSchemaVersion` 保存编辑值；descriptor 参数显式 `persistValues=false` 时既不从也不向 localStorage 读写，字段缺失仍兼容为 true。保存目录只由后端配置，曲线选择不改变保存列。`mbddf.do_write` 与 `mbddf.helm_board_test` 专用页面只消费当前完整快照的版本化 `rawData.boardTest` v1；数字量输出显示完整指令与 DUT 回读，但只把 PXI-6259 P0.3/P0.4 对 bit2/bit1 的结果称为外部物理验证，其他 DO 标为未配置外部回读；舵机板级页显示方向矩阵、自动 PWM/反馈的指令/实测/误差/容差和摘要或手动一次响应。板级结果不是 TXT、跨进程持久化或真机验收。`mbddf.helm_stream` 还投影 `postRunAnalysis` capability、`snapshot.analysis` 小型摘要和只读 `analysisResult` 按通道结果；浏览器性能页只消费后端投影，不以 50,000 点实时环形缓存计算性能。
- 舵机连续实测由 DUT 以 1 ms 周期经 DDS 连接用户独立启停的 `MB_DDF_v2_HelmControl`；27 字节 `helm_command` 的 B27/U8 为 `helm_unlock`，连续测试始终发送 `0xFF` 请求解锁。舵控程序启动先禁止 PWM，收到请求后将 DIDO DO0 置高，至少等待 30 ms 再使能 PWM；解锁在进程内单向锁存，STOP 只发布四路零位指令，不关舵锁或禁止 PWM。它与 `HELM_BOARD_TEST 07/02` 不建立生命周期、互斥、忙状态或进程控制绑定，`07/02` 不包含舵锁流程。DUT/Web 不设置舵角业务范围，舵控程序保留内部限幅。算法层提供正弦、方波、三角波、恒值和连续对数扫频五种后处理及扫频伯德图；应用层已接通“STOP 尾样本封存 → `queued` 起写门禁 → 硬件收尾栅栏 → 后台分析/持久化/取消”，并通过 capability、DTO、WebSocket `analysisResult` 和性能页投影结果。性能结果是独立 sidecar，不得改变采集 `Pass/Fail`、BIZ `TestResult`、采集错误码或硬件 STOP 语义。TUI 和 Qt GUI 本轮不增加性能 UI。
...
- 当前 NI 证据等级为 Fake NI-DAQmx 自动化，不等于真实 PXI-6259 或 PXI-6733。板级真机验收须在隔离、明确授权的台架记录真实 SDK/DLL、MAX 设备名和非 `CONFIGURE_ME` 序列号、共地、电平、隔离、端点接线、PXI-6259 读回/采样参数、PXI-6733 AO 写入与安全清零、长时运行及异常收尾结果。
