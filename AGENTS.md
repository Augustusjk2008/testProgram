...
- 算法位于 `src/algorithm/`，目标为 `hwtest_algorithm_mbddf`，提供 MB_DDF CSV 编解码和十二项已注册算法：`mbddf.system_status`、`mbddf.elec_health_status`、`mbddf.bus_loop`、`mbddf.bus_echo`、`mbddf.memperf`、`mbddf.spi_flash`、`mbddf.dh_pulse_config`、`mbddf.dh_ignite_stream`、`mbddf.timer_jitter`、`mbddf.di_read`、`mbddf.imu_stream`、`mbddf.helm_stream`。
- 十二份 `configs/mbddf_*.testcfg.json` 中，BUS LOOP、SPI Flash 和 DH 脉宽配置只声明 `single`，BUS ECHO 只声明 `pc_periodic`，系统状态、电气健康、内存、定时器和 DI 声明 `single` 与 `pc_periodic`，DH 点火、惯测与舵机连续实测只声明 `device_stream`。BIZ 提供 `single`、`pc_periodic` 和 `device_stream` 三种通用运行语义，配置能力以各 JSON 的 `reportFields.supportedRunModes` 为准；同一配置不得同时声明 `pc_periodic` 与 `device_stream`，字段缺失时只安全回退到 `single`，应用控制器拒绝启动未声明的模式。
